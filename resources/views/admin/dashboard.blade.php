...
<style>
  :root {
    ...
    --gold:#f5a623;--gold-light:#fff8e1;--blue:#1565c0;--blue-light:#e3f2fd;
    ...
    --shadow-sm:0 2px 10px rgba(0,0,0,.07);--shadow-md:0 6px 24px rgba(0,0,0,.11);
  ...
  .kpi-icon--gold   { background: var(--gold-light);   color: var(--orange); }
  .kpi-icon--purple { background: var(--purple-light); color: var(--purple); }
  .kpi-label { font-size: .75rem; color: var(--text-light); font-weight: 700; text-transform: uppercase; letter-spacing: .08em; margin-bottom: 4px; }
  .kpi-number { font-size: 2rem; font-weight: 900; color: var(--text); line-height: 1; margin-bottom: 6px; }
  .kpi-link { font-size: .75rem; font-weight: 700; text-decoration: none; display: inline-flex; align-items: center; gap: 5px; transition: gap .18s; }
  .kpi-link:hover { gap: 8px; }
  .kpi-link--green  { color: var(--green); }
  .kpi-link--blue   { color: var(--blue); }
  .kpi-link--gold   { color: var(--orange); }
  .kpi-link--purple { color: var(--purple); }

  /* ── BOTTOM GRID ── */
  .dashboard-grid { display: grid; grid-template-columns: 1fr 340px; gap: 24px; margin-bottom: 28px; }
  ...
  .card-title { font-size: .95rem; font-weight: 800; color: var(--text); }
  .select-period {
    border: 1px solid var(--border); border-radius: var(--radius-sm);
    background: var(--cream); font-size: .78rem; color: var(--text-mid);
    padding: 6px 12px; font-family: 'Nunito', sans-serif; cursor: pointer; outline: none;
  }
  .chart-area { height: 220px; position: relative; overflow: hidden; }
  canvas#visitesChart { width: 100% !important; height: 100% !important; }
  ...
  .info-card-title { font-size: .88rem; font-weight: 800; color: var(--text); margin-bottom: 16px; }
  ...
  .activity-icon--gold   { background: var(--gold-light);   color: var(--orange); }
  .activity-icon--purple { background: var(--purple-light); color: var(--purple); }
  .activity-icon--teal   { background: #e0f7fa; color: #00838f; }
  .activity-body { flex: 1; }
  .activity-title { font-size: .82rem; font-weight: 800; color: var(--text); line-height: 1.3; }
  .activity-sub   { font-size: .75rem; color: var(--text-mid); }
  .activity-time  { font-size: .7rem; color: var(--text-light); white-space: nowrap; flex-shrink: 0; }
  .see-all-link { font-size: .78rem; font-weight: 700; color: var(--green); text-decoration: none; display: inline-flex; align-items: center; gap: 5px; margin-top: 12px; }
  .see-all-link:hover { gap: 8px; }

  /* Tâches */
  .tasks-list { display: flex; flex-direction: column; gap: 10px; }
  .task-item { display: flex; align-items: center; justify-content: space-between; gap: 12px; }
  .task-left { display: flex; align-items: center; gap: 10px; }
  .task-check { width: 20px; height: 20px; border-radius: 50%; background: var(--green); display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
  .task-check i { font-size: .6rem; color: white; }
  .task-name { font-size: .83rem; font-weight: 700; color: var(--text); }
  .task-status { font-size: .75rem; font-weight: 800; }
  .task-status--green  { color: var(--green); }
  .task-status--blue   { color: var(--blue); }
  .task-status--teal   { color: #00838f; }
  .task-status--orange { color: var(--orange); }
  .btn-stats {
    display: flex; align-items: center; justify-content: center; gap: 10px;
    background: var(--green); color: white;
    padding: 13px; border-radius: var(--radius-sm);
    font-size: .82rem; font-weight: 800; text-transform: uppercase; letter-spacing: .05em;
    text-decoration: none; font-family: 'Nunito', sans-serif; transition: background .2s; margin-top: 8px;
  }
  .btn-stats:hover { background: var(--green-dark); }
  ...
  .quick-grid { display: grid; grid-template-columns: repeat(5,1fr); gap: 14px; margin-top: 16px; }
  .quick-item {
    display: flex; flex-direction: column; align-items: center; gap: 10px;
    padding: 16px 10px; border: 1px solid var(--border); border-radius: var(--radius-md);
    cursor: pointer; text-decoration: none; transition: all .25s;
    background: var(--cream);
  }
  .quick-item:hover { background: var(--green-light); border-color: var(--green-soft); transform: translateY(-2px); }
  .quick-icon {
    width: 48px; height: 48px; border-radius: var(--radius-sm);
    display: flex; align-items: center; justify-content: center; font-size: 1.3rem;
  }
  .quick-icon--green  { background: var(--green-light);  color: var(--green); }
  .quick-icon--blue   { background: var(--blue-light);   color: var(--blue); }
  .quick-icon--gold   { background: var(--gold-light);   color: var(--orange); }
  .quick-icon--purple { background: var(--purple-light); color: var(--purple); }
  .quick-icon--dark   { background: #eceff1; color: #455a64; }
  .quick-label { font-size: .75rem; font-weight: 700; color: var(--text-mid); text-align: center; line-height: 1.3; }

  .mini-stats-grid { display: grid; grid-template-columns: repeat(5,1fr); gap: 16px; }
  .mini-stat {
    background: var(--white); border: 1px solid var(--border);
    border-radius: var(--radius-md); padding: 16px;
    box-shadow: var(--shadow-sm);
  }
  .mini-stat-label { font-size: .72rem; color: var(--text-light); font-weight: 700; margin-bottom: 6px; }
  .mini-stat-number { font-size: 1.5rem; font-weight: 900; color: var(--text); line-height: 1; margin-bottom: 4px; }
  .mini-stat-trend { font-size: .72rem; font-weight: 700; }
  .mini-stat-trend--up   { color: var(--green); }
...
<div class="dashboard-grid">
  ...
  <div style="display:flex;flex-direction:column;gap:20px;">
    ...
    <div class="mini-stats-grid">
      @php
        $ministats = [
          ['label'=>'Actualités publiées','val'=>'125','trend'=>'+10 ce mois','up'=>true,'color'=>'#1b5e20','pts'=>[10,15,12,18,14,20,22]],
          ['label'=>'Projets en cours',   'val'=>'18', 'trend'=>'+2 ce mois', 'up'=>true,'color'=>'#1565c0','pts'=>[8,9,10,12,13,15,18]],
          ['label'=>'Membres',            'val'=>'520','trend'=>'+15 ce mois','up'=>true,'color'=>'#e65100','pts'=>[460,470,480,490,500,510,520]],
          ['label'=>'Taux d\'engagement', 'val'=>'68%','trend'=>'+5% ce mois','up'=>true,'color'=>'#6a1b9a','pts'=>[50,55,58,60,62,65,68]],
          ['label'=>'Messages non lus',   'val'=>'46', 'trend'=>'-3 ce mois', 'up'=>false,'color'=>'#e53935','pts'=>[60,55,52,50,48,49,46]],
        ];
      @endphp
      ...
      <div class="mini-stat">
        ...
        <div class="mini-sparkline">
          @php
            $pts = $s['pts']; $min = min($pts); $max = max($pts);
            $range = $max - $min ?: 1; $w = 100/(count($pts)-1);
            $path = '';
            foreach($pts as $i => $v) {
              $x = $i * $w;
              $y = 28 - (($v - $min) / $range) * 24;
  ...
  <div class="side-col">
    ...
    <div class="info-card">
      ...
      <div class="tasks-list">
        <div class="task-item">
          <div class="task-left"><div class="task-check"><i class="fas fa-check"></i></div><div class="task-name">Site en ligne</div></div>
          <div class="task-status task-status--green">Actif</div>
        </div>
        <div class="task-item">
          <div class="task-left"><div class="task-check"><i class="fas fa-check"></i></div><div class="task-name">Sauvegarde</div></div>
          <div class="task-status task-status--blue">À jour</div>
        </div>
        <div class="task-item">
          <div class="task-left"><div class="task-check"><i class="fas fa-check"></i></div><div class="task-name">Mises de données</div></div>
          <div class="task-status task-status--teal">Connectée</div>
        </div>
        <div class="task-item">
          <div class="task-left"><div class="task-check"><i class="fas fa-check"></i></div><div class="task-name">Espace disque</div></div>
...
(function() {
  ...
  new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        legend: { position: 'bottom', labels: { font: { family: 'Nunito', size: 12 }, boxWidth: 14, padding: 16 } },
        tooltip: { bodyFont: { family: 'Nunito' }, titleFont: { family: 'Nunito' } }
      },
      scales: {
        x: { grid: { display: false }, ticks: { font: { family: 'Nunito', size: 11 } } },
        y: { grid: { color: 'rgba(0,0,0,.05)' }, ticks: { font: { family: 'Nunito', size: 11 } } }
      }
    }
  });
})();
