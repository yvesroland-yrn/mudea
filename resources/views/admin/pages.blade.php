...
<style>
:root{
  --green:#1b5e20;--green-dark:#0a3d14;--green-light:#e8f5e9;--green-soft:#c8e6c9;
  --blue:#1565c0;--blue-light:#e3f2fd;--purple:#6a1b9a;--purple-light:#f3e5f5;
  --white:#ffffff;--cream:#f4f6f8;--border:#e0e8e4;
  --text:#1a2e25;--text-mid:#455d4f;--text-light:#7a9585;
  --shadow-sm:0 2px 10px rgba(0,0,0,.07);
  --radius-sm:8px;--radius-md:14px;--radius-lg:20px;
}
.page-toolbar{display:flex;align-items:center;justify-content:space-between;margin-bottom:22px;}
.page-toolbar h1{font-size:1.1rem;font-weight:800;color:var(--text);}
...
.btn-ghost{display:inline-flex;align-items:center;gap:7px;background:transparent;color:var(--text-mid);padding:9px 16px;border-radius:var(--radius-sm);font-size:.82rem;font-weight:800;border:1px solid var(--border);cursor:pointer;font-family:'Nunito',sans-serif;transition:all .18s;}
.btn-ghost:hover{border-color:var(--green);color:var(--green);background:var(--green-light);}
.btn-draft{display:inline-flex;align-items:center;gap:7px;background:#eceff1;color:#546e7a;padding:9px 16px;border-radius:var(--radius-sm);font-size:.82rem;font-weight:800;border:1px solid #cfd8dc;cursor:pointer;font-family:'Nunito',sans-serif;transition:all .18s;}
...
.data-table td{padding:13px 16px;border-bottom:1px solid var(--border);font-size:.85rem;color:var(--text-mid);vertical-align:middle;}
...
.btn-icon--view{color:var(--green);}.btn-icon--view:hover{background:var(--green-light);border-color:var(--green-soft);}
.overlay{position:fixed;inset:0;background:rgba(10,20,14,.52);display:flex;align-items:center;justify-content:center;z-index:1050;padding:16px;opacity:0;pointer-events:none;transition:opacity .22s;}
.overlay.open{opacity:1;pointer-events:all;}
.modal{background:var(--white);border-radius:var(--radius-lg);width:100%;max-width:700px;max-height:92vh;overflow-y:auto;box-shadow:0 24px 60px rgba(0,0,0,.2);transform:translateY(20px) scale(.98);transition:transform .25s cubic-bezier(.22,1,.36,1);}
.overlay.open .modal{transform:translateY(0) scale(1);}
.modal-header{display:flex;align-items:center;justify-content:space-between;padding:18px 22px 16px;border-bottom:1px solid var(--border);position:sticky;top:0;background:var(--white);z-index:2;}
.modal-header-left{display:flex;align-items:center;gap:10px;}
.modal-icon{width:36px;height:36px;border-radius:var(--radius-sm);background:var(--green-light);display:flex;align-items:center;justify-content:center;color:var(--green);font-size:1rem;flex-shrink:0;}
.modal-header h2{font-size:.95rem;font-weight:800;color:var(--text);margin:0;}
.modal-header p{font-size:.75rem;color:var(--text-light);margin:2px 0 0;}
.modal-close{width:30px;height:30px;border-radius:var(--radius-sm);border:1px solid var(--border);background:var(--cream);display:flex;align-items:center;justify-content:center;cursor:pointer;color:var(--text-mid);font-size:.9rem;transition:all .18s;flex-shrink:0;}
.modal-close:hover{background:#ffebee;color:#e53935;border-color:#ef9a9a;}
.modal-body{padding:20px 22px;}
.tab-bar{display:flex;gap:2px;background:var(--cream);border-radius:var(--radius-sm);padding:3px;margin-bottom:20px;}
.tab{flex:1;padding:8px;text-align:center;font-size:.75rem;font-weight:800;border-radius:6px;cursor:pointer;border:none;background:transparent;color:var(--text-light);font-family:'Nunito',sans-serif;transition:all .18s;}
.tab.active{background:var(--white);color:var(--green);box-shadow:0 1px 4px rgba(0,0,0,.08);}
.form-section{margin-bottom:22px;}
.form-section-title{font-size:.7rem;font-weight:900;text-transform:uppercase;letter-spacing:.1em;color:var(--text-light);margin-bottom:12px;display:flex;align-items:center;gap:7px;}
.form-section-title i{font-size:.85rem;}
.form-section-title::after{content:'';flex:1;height:1px;background:var(--border);}
.form-row{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px;}
.form-row.full{grid-template-columns:1fr;}
.field{display:flex;flex-direction:column;gap:5px;}
.field label{font-size:.72rem;font-weight:900;color:var(--text);text-transform:uppercase;letter-spacing:.07em;display:flex;align-items:center;gap:4px;}
.field label .req{color:#c62828;font-size:.8rem;}
.field input,.field select,.field textarea{border:1px solid var(--border);border-radius:var(--radius-sm);padding:9px 12px;font-size:.84rem;font-family:'Nunito',sans-serif;color:var(--text);background:var(--cream);outline:none;transition:border-color .18s,background .18s;}
.field input:focus,.field select:focus,.field textarea:focus{border-color:var(--green);background:var(--white);box-shadow:0 0 0 3px rgba(27,94,32,.08);}
...
.slug-prefix{display:flex;align-items:center;border:1px solid var(--border);border-radius:var(--radius-sm);overflow:hidden;background:var(--cream);}
.slug-prefix span{padding:9px 10px;font-size:.82rem;background:#e8ecea;color:var(--text-light);border-right:1px solid var(--border);white-space:nowrap;}
.slug-prefix input{border:none;background:transparent;padding:9px 12px;font-size:.84rem;font-family:'Nunito',sans-serif;color:var(--text);outline:none;flex:1;width:0;}
...
.toggle-label{display:flex;align-items:center;gap:8px;font-size:.83rem;font-weight:700;color:var(--text-mid);}
.toggle-label i{color:var(--text-light);font-size:.9rem;}
.toggle{position:relative;width:38px;height:22px;flex-shrink:0;}
.toggle input{opacity:0;width:0;height:0;position:absolute;}
.tslider{position:absolute;inset:0;background:#c8d5c4;border-radius:999px;cursor:pointer;transition:background .2s;}
.tslider::after{content:'';position:absolute;left:3px;top:3px;width:16px;height:16px;border-radius:50%;background:white;transition:transform .2s;box-shadow:0 1px 3px rgba(0,0,0,.2);}
.toggle input:checked+.tslider{background:var(--green);}
.toggle input:checked+.tslider::after{transform:translateX(16px);}
.modal-footer{display:flex;align-items:center;justify-content:flex-end;gap:10px;padding:14px 22px 18px;border-top:1px solid var(--border);background:var(--cream);position:sticky;bottom:0;z-index:2;}
</style>
@endpush
...
<div class="page-toolbar">
  <h1>Pages du site <span style="font-weight:600;color:var(--text-light);font-size:.9rem;">({{ count($pages ?? []) ?: 8 }})</span></h1>
...
<table class="data-table">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <code style="background:var(--cream);padding:2px 8px;border-radius:4px;font-size:.78rem;">{{ $p['slug'] }}</code>
      </td>
      <td>
      ...
      <td style="color:var(--text-light);font-size:.78rem;">{{ now()->subDays(rand(1,30))->format('d M Y') }}</td>
      <td>
...
{{-- ═══════════════════════════════ MODAL ═══════════════════════════════ --}}
<div class="overlay" id="modal-overlay" onclick="if(event.target===this)closePagesModal()">
  <div class="modal" role="dialog" aria-modal="true" aria-labelledby="modal-title">
    ...
    <div class="modal-header">
      <div class="modal-header-left">
        ...
        <div>
          <h2 id="modal-title">Nouvelle page</h2>
          <p>Créez une page statique pour le site</p>
        </div>
      </div>
      <button class="modal-close" onclick="closePagesModal()" aria-label="Fermer">
    ...
    <div class="modal-body">
      ...
      {{-- ✅ FIX 1 : id="form-page" ajouté + action correcte --}}
      <form id="" method="POST">
        @csrf
          ...
          <div class="form-section">
            ...
            <div class="form-row full">
              <div class="field">
                ...
                <div class="slug-prefix">
                  <span>mudea.ci</span>
                  <input type="text" id="p-slug-input" name="slug" placeholder="/ma-page">
                </div>
            ...
            <div class="form-row">
              <div class="field">
                ...
                {{-- ✅ FIX 2 : onchange synchronise le champ hidden --}}
                <select id="p-statut-select" onchange="document.getElementById('p-statut').value=this.value">
    ...
    <div class="modal-footer">
      <button type="button" class="btn-draft" id="p-btn-brouillon">
        <i class="fas fa-save"></i> Brouillon
      </button>
      <button type="button" class="btn-ghost" onclick="closePagesModal()">Annuler</button>
      <button type="button" class="btn-primary" id="p-btn-publier">
...
document.querySelectorAll('.tab-bar .tab').forEach(function (tab) {
  tab.addEventListener('click', function () {
    ...
    ['general', 'contenu', 'seo'].forEach(function (id) {
...
function pSyncSlug(v) {
  var s = '/' + v.toLowerCase()
    .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
    .replace(/[^a-z0-9\s-]/g, '').trim().replace(/\s+/g, '-');
  document.getElementById('p-slug-input').value = s;
  document.getElementById('mp-slug').textContent = s.replace('/', '');
  if (!document.getElementById('p-meta-title').value) {
    document.getElementById('mp-title').textContent = (v || 'Titre de la page') + ' — MUDEA';
  }
}

/* ── Aperçu Google ── */
function pUpdateMeta() {
...
/* ── Brouillon ── */
document.getElementById('p-btn-brouillon').addEventListener('click', function () {
  document.getElementById('p-statut').value = 'brouillon';
  document.getElementById('form-page').submit();
});

/* ── Publier ── */
document.getElementById('p-btn-publier').addEventListener('click', function () {
  var n = document.getElementById('p-nom').value.trim();
  var s = document.getElementById('p-slug-input').value.trim();
  if (!n || !s) {
    alert('Veuillez renseigner le nom et le slug de la page.');
    return;
  }
